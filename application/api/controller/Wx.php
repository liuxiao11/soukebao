...
class Wx extends Rest
{
    ...
    //微信支付
    public function Wx_Payphone()
    {
        $request = Request::instance();
        $fee = $request->param('total');
        $details = '查询电话号码';//商品的名称
        $program = config('pay');
        $appid = $program['app_id'];//appid
        $body = $details;// '商品信息';//'【自己填写】'
        $mch_id = $program['mch_id'];//'你的商户号【自己填写】'
        $nonce_str = $this->nonce_str();//随机字符串
        $notify_url = 'http://www.soukebao.com/wxSpeechphone';//回调的url【自己填写】';
        $openid = openId($request->param('code'));//'用户的openid【自己填写】';
        $out_trade_no = '1298016501' . date("Ymd") . rand(10000, 90000) . rand(10000, 90000);//商户订单号
        $spbill_create_ip = '192.168.1.196';//'服务器的ip【自己填写】';
        $total_fee = $fee * 100;//因为充值金额最小是1 而且单位为分 如果是充值1元所以这里需要*100
        ...
        $array = $this->xml($xml);//全要大写
        if ($array['RETURN_CODE'] == 'SUCCESS' && $array['RESULT_CODE'] == 'SUCCESS') {
            $find = findone('get_house',[],'pay_id',['id'=>$request->param('msg_id')]);
            if($find != "0"){
                $pay_id = $find['pay_id'].",".$request->param('user_id');
            }else{
                $pay_id = $request->param('user_id');
            }
            edit('get_house',['id'=>$request->param('msg_id')],['pay_id'=>$pay_id]);
            $time = time();
            $tmp = [];//临时数组用于签名
            $tmp['appId'] = $appid;
            $tmp['nonceStr'] = $nonce_str;
    ...
    public function sign($data)
    {
        ...
        $wx_key = 'zhaoxiaoyong66610124198702201816';//申请支付后有给予一个商户账号和密码，登陆后自己设置key
